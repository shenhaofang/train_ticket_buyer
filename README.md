...
- 这里列出直接从12306导出的站点对于编码（导出于2025-09-13）：
    - 北京北: VAP
    - 北京东: BOP
    - 北京: BJP
    - 北京南: VNP
...
- 无头模式
    ...
    - 如果config.json中headless为true则打开无头模式，运行后会自动打开chrome无头浏览器（看不到浏览器界面）并输入账号密码，收到手机验证码后，需在captcha.txt中输入验证码（后续可以考虑用接码服务接到验证码自动填入），脚本会继续运行，直到购买成功
